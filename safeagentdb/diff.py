...
class RowDiff:
    ...
    def blocked_generated_columns(self) -> list[str]:
        """Columns on this INSERT whose value only production can generate.

        The sandbox clone lost the production default (a sequence, an identity
        column, a uuid function), so SQLite either left the column NULL or --
        for an integer primary key, which SQLite makes an alias for rowid --
        quietly invented a value from the tenant's cloned rows alone. Neither
        may reach production, and after the fact the two are indistinguishable,
        so a generated key column blocks the INSERT either way.
        ...
        return [
            ...
            if col in self.pk or row_data.get(col) is None
        ]
